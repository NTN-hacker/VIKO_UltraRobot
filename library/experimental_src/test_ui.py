...
import os
import signal
from infer.IPCDataMs import IPCData
import math
...
class CameraPanel(wx.Panel):
    def __init__(self, parent):
        ...
        # self.send_thread = threading.Thread(target=self.receive_button)
    ...
    def send_frame(self):
        # Send Frame to the Inter-Process Communication [Shared Memory]; 
        # Pause ms for avoiding overhead transfer data;
        while self.running:
            time.sleep(33/1000)   
            img_str = self.img
            img_np = np.frombuffer(img_str, dtype=np.uint8).reshape((640, 640, 3))
            self.ipc_data.send_frame(img_np, self.frame_count)
            self.frame_count += 1         
    ...
    def receive_button(self):
        ...
        while self.running:
            ...
            print(trigger_list)
            # get trigger
            for idx, value in enumerate(trigger_list):
                if value == 1:
                    frame = InspectionFrame(None, "test")
                    if idx == 0: 
                        frame.on_run_inspection_plan()            
                    elif idx == 1:
                        frame.on_start_robot()            
                    elif idx == 2:
                        frame.on_stop_robot()            
                    elif idx == 3: 
                        frame.on_view()            
            ...
            with self.lock:
                if not self.capturing_image:  # Kiểm tra cờ trước khi lấy ảnh
                    ...
                    if grabResult.GrabSucceeded():
                        ...
                        img_np = np.frombuffer(img, dtype=np.uint8).reshape((640, 640, 3))
                        self.ipc_data.send_frame(img_np, self.frame_count)
                        self.frame_count += 1
                        time.sleep(33/1000)              
                        #######
...
class Infer():
    ...
    def on_run_inspection_plan(self):
        # LaserTrigger()
    ...
    def view(self):
        ...
        lib.process_temp(data_path= data_path)
        img = cv2.imread('laser/laser_shader-.png', cv2.IMREAD_ANYCOLOR)
